...
cat $filename | cut -f $x_column,$y_column | awk 'BEGIN {FS="\t"; print("v_call\tj_call")} /IG|TR/ {printf("%s\t%s\n",substr($1,0,index($1,"*")-1), substr($2,0,index($2,"*")-1))}' > $filename2
...
python3 airr_heatmap.py v_call j_call $xvals $yvals $filename2 $x_column_str-$y_column_str-sample$sample.png
